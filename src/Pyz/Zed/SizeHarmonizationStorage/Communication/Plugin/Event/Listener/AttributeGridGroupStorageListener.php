 ...
 */
class AttributeGridGroupStorageListener extends AbstractPlugin implements EventBulkHandlerInterface
{
    /**
    ...
    public function handleBulk(array $eventTransfers, $eventName)
    {
        ...
        $productAbstractIds = $this->getQueryContainer()
            ->queryProductAbstractIdsByAGGroupIds($messageIds)
            ->distinct()
            ->find()
            ->getData();

        if ($eventName === SizeHarmonizationEvents::ENTITY_MYT_ATTRIBUTE_GRID_GROUP_DELETE) {
            $this->getFacade()->unpublishAttributeGrid($productAbstractIds);
        } elseif ($eventName === SizeHarmonizationEvents::ENTITY_MYT_ATTRIBUTE_GRID_GROUP_CREATE
            || $eventName === SizeHarmonizationEvents::ENTITY_MYT_ATTRIBUTE_GRID_GROUP_UPDATE
        ) {
